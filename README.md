...
3. 「实例对比与同步」页按**意图**分两档（读写分离）：**对比（只读）** 与 **同步（写入）**。范围（单个对象 / 全部对象）不是独立开关，而是**对象选择框里的子选项**——首项「全部对象（N 个）」= 整类批量，其余为具体对象，选谁就作用于谁。写入口只在「同步」档出现，「对比」档没有任何写按钮。
   - **对比 · 单个对象**：选源与对象，「开始对比」出字段级差异。自定义应用/URL 做字段级对比（IP/域名/URL 列表做行级增删、相同项弱化折叠）；访问权限策略按规则顺序对位，**每条规则（含「一致」的）都展开内容**，把引用分「自定义应用 / 内置应用 / 自定义URL库 / 内置URL库」四类列出并标注**动作（放行/拒绝）**，源有目标无=绿、目标有源无=红删除线、共有=灰。
   ...
   - **同步 · 全部对象（批量）**：一次把源某类对象（全部应用/URL/策略）同步到目标。批量为**全量 upsert**：目标无同名→新增，已有同名→**覆盖**为与源一致（**不逐项比对内容**——要看内容差异请用「对比」档）。结果按目标分组显示新增/覆盖/删除/失败数与名单。**「同步预览」(dry-run) 只按名称分类、不逐个拉详情**，所以很快；真实写入才逐个建/改/删。可勾选**镜像模式**——删除目标上「源没有」的同类对象（仅自定义对象；内置 URL 库不受影响）；镜像开启时确认弹窗会红色警示、务必先「仅预览」核对删除清单。
