...
| 5 | 跨实例同步 | **单对象**：选源选目标预览差异 / 二次确认后真实写入（应用·URL 直写；策略据目标 crc 重映射，缺失的自定义引用自动先建、内置缺失则跳过并告警）。**批量**：一次同步整类对象（全部应用/URL/策略），可选**镜像模式**删除目标上「源没有」的对象 | 组合上述接口 |
...
3. 「实例同步」顶部可切换**同步范围**：
   - **单个对象**：选源实例与对象（对象名称为带搜索的下拉框），预览差异后同步到目标或一键推送全部。差异预览按对象友好展示：自定义应用/URL 做字段级对比（IP/域名/URL 列表做行级增删、相同项弱化折叠）；访问权限策略按规则顺序对位，**每条规则（含「一致」的）都展开内容**，把引用分「自定义应用 / 内置应用 / 自定义URL库 / 内置URL库」四类列出，源有目标无=绿、目标有源无=红删除线、共有=灰；卡片标题显示「源 → 目标」。
   - **全部对象（批量）**：一次把源实例某类对象（全部应用/URL/策略）同步到目标。批量为**全量 upsert**：目标无同名→新增，已有同名→**覆盖**为与源一致（**不逐项比对内容**——要看内容差异请用「单个对象」模式）。结果按目标分组显示新增/覆盖/删除/失败数与名单。**预览（dry-run）只按名称分类、不逐个拉详情**，所以很快；真实写入才逐个建/改/删。可勾选**镜像模式**——删除目标上「源没有」的同类对象（仅自定义对象；内置 URL 库不受影响）；镜像开启时确认弹窗会红色警示、务必先「仅预览」核对删除清单。
   - 两种范围都走二次确认弹窗，可选「仅预览（dry-run）」或「确认写入」；改动左侧配置后右侧预览不清空而是标记「已过期」并提示重新预览。
...
5. 「操作日志」查看所有写 / 同步动作的审计记录。每条都带可读「摘要」（如「新增规则：拒绝 → 钉钉、微信」「新增 URL 库『test』（2 条 URL/IP）：1.1.1.1、example.com」「新增自定义应用『app』（IP 2 条：…）」）；顶部搜索框对摘要 / 对象 / 操作人 / 实例 / 变更前后快照做子串检索——可按你添加过的 IP、域名、URL 等直接搜到相关记录。策略编辑的「变更前/后」存精简的「规则=动作+引用」快照而非整条原始报文。
...
> - **引用对象自动创建 / 更新 + 最大努力同步**：策略引用的对象若目标缺失，会区分「自定义」与「内置」——**自定义**应用 / URL 库复用已确认写接口同步到目标：**目标已有同名对象则改成与源一致（update），没有才新增（add）**，避免对已存在对象误走 add 报「名字已被使用」；同一对象被多个引用路径命中（如一个 URL 库的多个子类）只处理一次（去重）；含「自定义」前缀与应用名粘连成一段如 `自定义应用_AIGC应用` 的识别。缺失的**内置**对象无法创建（多因目标应用库版本较旧），会**跳过这些引用、尽可能同步其余内容**，并在结果中列出被跳过的引用，不阻断整条策略同步。
