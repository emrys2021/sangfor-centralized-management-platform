...
class BatchSyncRequest(BaseModel):
    ...
    allow_degrade: bool = False
    # 只推送「已选」的一部分对象：None=推送源上全部；给定列表=只推这些；空列表 []=一个都不推
    # （用于「只删不推」的场景）。与 mirror 互斥——镜像要求以源的**全集**为基准判断目标多余对象，
    # 选中子集时贸然镜像会把「没被选中但双方都合法存在」的目标对象误删，故服务层会在两者同时为真时拒绝。
    object_names: list[str] | None = None
    # 选择性删除：只删除**指定**的目标多余对象（来自对比结果「仅目标有」分组的勾选）。走与镜像
    # 同一套安全闸（策略在用则拒删、删前存快照、落审计）；只允许删「确属目标多余」的对象，传入的
    # 名字若源上也有或目标上不存在会被跳过记 failed。与 mirror 互斥（镜像已删全部多余）。
